...
def create_kmeans_input(data_arrays: dict) -> typing.ArrayLike:
    ...
    ----------
    data_arrays : dict
        Output of kmeans.read_and_preprocess_data(). Dictionary where keys are dataset 
    ...
    See Also
    --------
    kmeans.read_and_preprocess_data : Reads and preprocesses data for each dataset
...
def plot_cluster_trj_data(input_file: str, n_cluster: int, outfile: str, seeded: bool = False) -> None:
    ...
    Examples
    --------
    >>> plot_cluster_trj_data('clustering_results.txt', 4, '/path/to/output/', True)
